...
Repository policy is selected in `ras-stack.policy.json`. Run `node dist/policy/cli.js sync` after building when the shared policy or its local selection changes; `just check` rejects drift.
