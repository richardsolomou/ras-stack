...
Repository policy is selected in `ras-stack.policy.json`. Run `node packages/ras-stack/dist/cli.js policy sync` after changing generated policy; `just check` rejects drift.
...
```sh
test "$(node -p "require('./packages/ras-stack/package.json').version")" = "0.40.0"
